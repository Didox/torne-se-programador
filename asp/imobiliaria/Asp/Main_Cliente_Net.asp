...
if request("flag") = 1 then
   ...
   'Data de emissao nao esta sendo utilizada
   ...
   if msg="" then
	  ...
	  If identidade <> "" and orgao_emissor <> ""  then 
        sqlins = sqlins & " ,identidade,orgao_emissor"
      End If
	  
	  IF data_emissao <> "" THEN
	  	    sqlins = sqlins & " ,data_emissao"
	  END IF
	  
      sqlins = sqlins & ", data_nascimento, email,data_inclusao,status,senha,login,responsavel,creci,sexo)" 
	  
      sqlins = sqlins & " values ("&tipo1&",'"&nome&"','"&endereco&"','"&numero&"','"&complemento&"','"&bairro&"','"&cidade&"','"&estado&"','"&cep&"','"&ddd&"','"&fone&"','"&ddd_cel&"','"&celular&"','"&cpf&"'"
	  ...
	  IF data_emissao <> "" THEN
	  	    sqlins = sqlins & ",'"&year(data_emissao)&"/"&month(data_emissao)&"/"&day(data_emissao)&"'"
	  END IF
	  
      sqlins = sqlins & ",'"&year(data_nascimento)&"/"&month(data_nascimento)&"/"&day(data_nascimento)&"','"&email&"', getdate() ,2,'"&senha2&"','"&login2&"','"&responsavel&"','"&creci&"',"&sexo&")"
          ...
          <td>
            <table cellspacing=0 cellpadding=0 border=0>
              <tr>
                ...
                <!--td data de emissao nao esta sendo utilizada>
              ...
              <tr>
                ...
                <!--Alteração a linha de data de emissao do RG nao e utilizada-->
				<!--td>
              ...
              <tr>
                <td colspan=5>
                  <input type="text" name="cpf" value="<%=cpf%>"  maxlength=18 label="cpf" style="width:145; height:17;">
                </td>
              </tr>	 
              <tr>
                <td colspan=5>
