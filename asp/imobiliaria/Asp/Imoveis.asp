...
if request.form("estado")<>"" and request.form("estado")<>"0" then
  sql = sql &" and imoveis.cod_estado = "& request.form("Estado")
End if

if request("PubSim")<>"" then
  sql = sql &" and imoveis.publicar = "& request("PubSim")
End if

'if request("ImovVencSim")<>"" then
'  sql = sql &" and imoveis.ImovVencSim = "& request("ImovVencSim")
'End if

if request("ImovPendSim")<>"" then
  sql = sql &" and imoveis.divida = "& request("ImovPendSim")
End if

If request.form("estado")<>"" and request.form("cidade")<>"" and request.form("estado")<>"0" and request.form("cidade")<>"0"  then
  sql = sql &" and imoveis.cod_cidade ="& request.form("cidade")
End if
...
'response.Write("select * from imoveis  where status in (1,2) "&sql&sql_filtro&" ORDER BY "&ordem&" "&ordem2)


itens.Open "select * from imoveis  where status in (1,2) "&sql&sql_filtro&" ORDER BY "&ordem&" "&ordem2,conn


strBuscaImov = strBuscaImov & "AND destaque=1"&sql_filtro
'=========================================================================
      ...
      <!--#INCLUDE FILE ="Main_Topo.asp"-->
          ...
          <table width=100% cellpadding="4">
            ...
            <tr>
              ...
              <td valign=top>
			  <!--1º Busca por Nome Imoveis-->
			  <input type=text name="pesquisa" value="<% if trim(request("pesquisa"))<>"" then  response.write(request("pesquisa")) else response.write(pesquisa) end if%>" style="width:190; height:17; background=#E8E8E8;">			  		  
			  </td>
			...
			<td valign=top>
			  <!--2º Busca por Estado-->Filtrar por Estado:<br>
			    <select name="estado" style="width:140;background=#E8E8E8;" onChange="PassaEstado();">
                    <option value = "0">(Selecione)</option>
                    <%set cbai=conn.execute("select * from estado where status=0 ORDER BY descricao")%>
                    <%do while not cbai.eof %>
                      <%if cstr(cbai("codigo"))= request("estado") then%>
			...
			<%
			'Opção de filtro por Cidade somente é exibido com a seleção do Estado
			If trim(request.QueryString("estado"))<>"" then
			%>
			<tr>
				<td>&nbsp;
			    
				</td>
				<td>Filtrar por Cidade<br>
				   <select name="cidade" style="width:140;background=#E8E8E8;" onChange="">
                    <option value = "0">(Selecione)</option>
                    <%set cbai=conn.execute("select * from cidade where status=0 and cod_estado="&request.QueryString("estado")&" ORDER BY descricao")%>
                    <%do while not cbai.eof %>
			...
			<td>
			Imoveis Publicados:<br>
			Sim<input type="radio" name="PubSim" value="1">  Não<input type="radio" name="PubSim" value="0"><br>
			<!--Imoveis Vencidos:<br>
			Sim<input type="radio" name="ImovVencSim" value="1"> Não<input type="radio" name="ImovVencSim" value="0"><br>-->
			Imoveis com Pendências:<br>
			Sim<input type="radio" name="ImovPendSim" value="1"> Não<input type="radio" name="ImovPendSim" value="0">
...
function PassaEstado()
	{
    var pesquisa 	= document.form1.pesquisa.value;
	var estado 		= document.form1.estado.value;
	window.location.href='Imoveis.asp?codigo=<%=request("codigo")%>&estado='+estado+'&pesquisa='+pesquisa;
	}
</script>	
