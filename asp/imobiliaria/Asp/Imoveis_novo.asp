...
if box <> "" then
   conn.Execute ("update Imoveis set status="&request("status")&" where codigo IN ("&box&")")
end if
      ...
      <!--#INCLUDE FILE ="Main_Topo.asp"-->
          ...
          <table width=100%>
            <tr>
              ...
              <td align=center width=70 bgcolor=#B0B0B0><font color=#FFFFFF><b>Ações</b></td>
            </tr>
                  ...
                  <tr>
                    ...
                    <td align="center" class="tb" bgcolor=<%=cor%>>
                      ...
                      <%if tipo_login="2" then%>
                        <a href="javascript:confirma2('<%=request.servervariables("SCRIPT_NaME")%>?currentPage=<%=currentpage%>&ordem=<%=ordem%>&ordem2=<%=ordem2%>&deletar=<%=itens("codigo")%>&pesquisa=<%=pesquisa%>&opcao=<%=opcao%>')"><img src="../img/excluir_n.gif" alt="Indisponibilizar Imóvel" border=0></a>
                      ...
                      <%end if%>  
                      <%else%>
                        <a href="<%=request.servervariables("SCRIPT_NaME")%>?currentPage=<%=currentpage%>&ordem=<%=ordem%>&ordem2=<%=ordem2%>&deletar=<%=itens("codigo")%>&pesquisa=<%=pesquisa%>&opcao=<%=opcao%>&status=2"><img src="../img/excluir_n.gif" alt="Disponibilizar Imóvel" border=0></a>
                      <%end if%>
                    </td>                    
                  </tr>
                  <%cont=cont+1%>
