...
if isdate(data_inicio) and isdate(data_final) and servico <> "" then%>
   <script>
     location.href="relatorio_produtividade.asp?pagina=relatorio_produtividade_dados&data_inicio=<%=data_inicio%>&data_final=<%=data_final%>&servico=<%=servico%>";
   </script>
<%end if%>
<html>
<head>
      ...
      <!--#INCLUDE FILE ="Main_Topo.asp"-->
          ...
          <table width=100% border=0 bgcolor=#E8E8E8 cellspacing="0" cellpadding="0">
            <tr> 
              ...
              <td class="tb"><B>RELATÓRIO PRODUTIVIDADE</B></td>
            </tr>
          </table>  
          <table border=0>
            <tr>
            ...
            <tr> 
              <td class="tb" align=center>Serviço</td>
            </tr>
            <tr>
              <td class="tb">
                <select name="Servico" style="width:140;background=#E8E8E8;">
                 <option value= ""></option>
                  <%set ccid=conn.execute("select * from Servico where status=0 ORDER BY descricao")%>
                   <%do while not ccid.eof %>
                    <%if cstr(ccid("codigo"))=Servico then%>
