...
if reload="2" and excluir="" and isnumeric(valor) then
   ...
   'response.write ("insert into perfil_servico (cod_imovel, cod_servico, valor, cod_moeda) values ("&codigo&", "&servico&", "&replace(replace(csng(valor),".",""),",",".")&","&moeda&")")
   'response.end
...
if reload="6" and excluir="" and isnumeric(empreendimento) and isnumeric(quantidade_empreendimento) then
   ...
   alert('O Iten de InfraEstrutura foi Inserido com Sucesso!');
   </script>  
   <%
...
if reload="7" and excluir="" and isnumeric(lazer) then
   ...
   %>
   <script language="javaScript">
   alert('O Iten de Lazer foi Inserido com Sucesso!');
   </script>
   <%
end if
if reload="8" and excluir="" and isnumeric(seguranca) then
   ...
   conn.execute("insert into perfil_seguranca (cod_imovel, cod_seguranca) values ("&codigo&", "&seguranca&")")
    %>
   <script language="javaScript">
   alert('O Iten de Segurança foi Inserido com Sucesso!');
   </script>
  <% 
end if
...
if request("flag") <> 1 then
   ...
   set servico_plano = conn.execute("select * from perfil_servico where cod_imovel="&codigo)
   ...
   '=================================================================================================
   'Ajuste Subtipo_Imoveis em 10/02/2006
      ...
      <table border="0" width=376 align=right>
        ...
        <tr>
          ...
          <table cellspacing=0 cellpadding=0>
              ...
              <tr>
                ...
                <td>
                  <input type="text" name="valor" value="<%if request("valor")<>"" then response.write(valor) else response.write(request("valor")) end if%>" label="valor" style="width:60; height:17; background=#E8E8E8;" onkeyup="Numero(this.value,this)">&nbsp;&nbsp;&nbsp;&nbsp;
                </td>
                <td align=center>
            ...
            <table width=100% cellspacing=0 cellpadding=0 border=1 bordercolor="#F8F8F8">
              ...
              <%set cbai=conn.execute("select * from perfil_servico where cod_imovel="&codigo&" ORDER BY codigo desc")%>
              <%do while not cbai.eof %>
              <tr>
                <td>
        ...
        <tr>
         ...
         <tr>
				         ...
				         <tr>
				          <td>
							<%
							if request("Acao")<>"Captura" then 
							%>
							<select name="tipo_imovel" style="width:140;background=#E8E8E8;" onChange="javascript:PassaTipo();">
						  ...
						  <select name="tipo_imovel" style="width:140;background=#E8E8E8;" onChange="javascript:PassaTipo();">
				            <%set cbai=conn.execute("select * from tipo_imovel where status=0 ORDER BY descricao")%>
				              <%do while not cbai.eof %>
				                <%if cstr(cbai("codigo")) = request("tipo_imovel") then%>
								    <option value="<%=cbai("codigo")%>" selected><%=cbai("descricao")%>					               
									<%else%>
				                  <option value="<%=cbai("codigo")%>"><%=cbai("descricao")%>
				                <%end if%>
        ...
        <tr>
         ...
         <tr>
          <td>
<%
if request("Acao")<>"Captura" then ' Quando a ação é diferente de capturar ele ja traz o dado do banco e deixa selected
					   'Traz dinâmicamente o Subtipo relacionado com o Tipo

					   Sel = "select subtipo_imovel from imoveis where codigo = "& codigo
					   
					   
					   
					   Set SelSubtipo = conn.execute(Sel)
					  ' response.write ("aqui")
					   
					   StrSubImovel = "select * from tipo_subtipo_imovel where tipo_imovel = "& tipo_imovel' query de relacionamento entre a tabela tipo & Subtipo imovel
					   'response.write StrSubImovel
					   Set SubTipo = conn.execute(StrSubImovel)
%>
					   <select name="subtipo_imovel" style="width:140;background=#E8E8E8;">
			              <option value = "">(Selecione)</option>
			              <%do while not SubTipo.eof %>                
			                  <option value="<%=SubTipo("codigo")%>" <%if trim(Imoveis("subtipo_imovel"))=trim(SubTipo("codigo"))then%>Selected<%end if%>><%=SubTipo("descricao")%></option>				                 
			                <%SubTipo.movenext%>
			               <%loop%>            
			            </select>
<%
Else ' quando na tela de edição o usuario muda o tipo do imovel
%>
<%
					   'Traz dinâmicamente o Subtipo relacionado com o Tipo
					   ...
					   StrSubImovel = "select * from tipo_subtipo_imovel where tipo_imovel = "& request("tipo_imovel")' query de relacionamento entre a tabela tipo & Subtipo imovel
					   'response.write StrSubImovel
					   Set SubTipo = conn.execute(StrSubImovel)
%>
					   <select name="subtipo_imovel" style="width:140;background=#E8E8E8;">
			              <option value = "">(Selecione)</option>
			              <%do while not SubTipo.eof %>                
			                  <option value="<%=SubTipo("codigo")%>" <%if trim(Imoveis("subtipo_imovel"))=trim(request("subtipo_imovel"))then%>Selected<%end if%>><%=SubTipo("descricao")%></option>				                 
			                <%SubTipo.movenext%>
			               <%loop%>            
			            </select>
<%
...
%>						
		 ...
		 %>
		     ...
		     <select name="Categoria" style="background=#E8E8E8;">
	            <option value = "0"></option>
	              <%set cbai=conn.execute("select * from tipo_Categoria where status=0 and tipo_imovel="&tipo_imovel&" ORDER BY descricao")%>
	              <%do while not cbai.eof %>
			...
			%>
			 <select name="Categoria" style="background=#E8E8E8;">
	            <option value = "">(Selecione)</option>
	              <%set cbai=conn.execute("select * from tipo_Categoria where tipo_imovel=" &request("tipo_imovel")& "ORDER BY descricao")%>
	              <%do while not cbai.eof %>
	                <%if cstr(cbai("codigo"))=cstr(Categoria) then%>
	                  <option value="<%=cbai("codigo")%>" selected><%=cbai("descricao")%>
	                <%else%>
...
function PassaTipo()
	{
		...
		var servico 			= document.form1.servico.value;
		...
		window.location.href='Alt_Imoveis.asp?cliente=<%=request("cliente")%>&pagina=<%=request("pagina")%>&estado='+estado+'&cidade='+cidade+'&logradouro='+logradouro+'&numero='+numero+'&complemento='+complemento+'&bairro='+bairro+'&servico='+servico+'&valor='+valor+'&tipo_imovel='+tipo_imovel+'&codigo=<%=codigo%>&Acao=Captura';
