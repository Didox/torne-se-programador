...
if request("flag") = 1 then
   ...
   if msg="" and reload = "" then
      conn.execute("insert into acompanhamento (cod_corretor , cod_imovel , cod_visita , cod_acompanhamento , descricao, data, acompanhamento) values ("&codigo2&","&imovel&","&codigo&","&tipo&",'"&descricao&"', convert(datetime,'"&data&"',103),1)")%>
      <script>
  ...
  <tr>
    <td>
      ...
      <table border="0" align=left width=380 cellspacing="0" cellpadding="0">
        ...
        <tr>
          <td>
              &nbsp;&nbsp;<a href="<%=pagina%>.asp?codigo=<%=codigo%>&codigo2=<%=codigo2%>&ordem=<%=ordem%>&ordem2=<%=ordem2%>&imovel=<%=imovel%>"><img src="../img/bot_voltar.gif" title="Voltar" border=0></a>
          </td>
        </tr>
      </table>   
       ...
       <tr>
        ...
        <tr>
          <td>
           ...
           <tr>
            <td>
              <%set corretor=conn.execute("select * from corretor where codigo="&codigo2)%>
              <b><%=corretor("nome")%></b>
            </td>
         </tr>
        </tr>
