...
set acompanhamentos=conn.execute("select * from acompanhamento where codigo="&acomp)
if request("flag") <> 1 then
   descricao=acompanhamentos("descricao")
   tipo=trim(ucase(acompanhamentos("cod_acompanhamento")))
   corretor=trim(ucase(acompanhamentos("cod_corretor")))
   imovel=trim(ucase(acompanhamentos("cod_imovel")))
   acompanhamento=trim(ucase(acompanhamentos("acompanhamento")))
   tempo=trim(ucase(acompanhamentos("data")))
else
   descricao=replace(trim(ucase(request("descricao"))),"'","")
   tipo=trim(ucase(request("tipo")))
   corretor=trim(ucase(request("c")))
   imovel=trim(ucase(request("imovel")))
   cliente=trim(ucase(request("cliente")))
   tempo=trim(ucase(request("tempo")))
   ...
   if msg="" and reload = "" then
     conn.execute("update acompanhamento set cod_imovel="&imovel&" , cod_corretor="&codigo2&" , cod_acompanhamento="&tipo&" , data=convert(datetime,'"&tempo&"',103) , descricao='"&descricao&"', acompanhamento="&acomp&" where codigo="&acomp)%>
      <script>
         alert('Alteração efetuada com sucesso!');
         location.href="<%=pagina%>.asp?codigo=<%=codigo%>&codigo2=<%=codigo2%>&ordem=<%=ordem%>&ordem2=<%=ordem2%>&imovel=<%=imovel%>";
      </script>
   <%end if
end if%>
<html>
...
<form method=post action="<%=request.servervariables("SCRIPT_NAME")%>?flag=1&pagina=<%=pagina%>&codigo=<%=codigo%>&codigo2=<%=codigo2%>&acomp=<%=acomp%>&tipo=<%=tipo%>" name="form1">
  ...
  <tr>
    <td>
      ...
      <table border="0" align=left width=380 cellspacing="0" cellpadding="0">
        ...
        <tr>
          <td>
            &nbsp;&nbsp;<a href="<%=pagina%>.asp?codigo=<%=codigo%>&imovel=<%=imovel%>&codigo2=<%=codigo2%>&pagina=Detalhe_Visita&ordem=<%=ordem%>&ordem2=<%=ordem2%>"><img src="../img/bot_voltar.gif" title="Voltar" border=0></a>
          </td>
        </tr>
      </table>   
    ...
    <table cellspacing=0 cellpadding=0>
       ...
       <tr>
         <td>
           ...
           <b><span class="estilo"><%=imov("descricao")%></b>
         </td>
       </tr>
      </table>
    </td>
