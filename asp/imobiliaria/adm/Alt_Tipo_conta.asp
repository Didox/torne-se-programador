...
if request("flag") <> 1 then
   set tipo_conta=conn.execute("select * from tipo_conta where tipo=1 and codigo="&codigo)
   descricao=replace(trim(ucase(tipo_conta("descricao"))),"'","")
else
   ...
   if descricao<>"" then
      ...
      if not verifica_descricao.eof then
         msg=msg&" Tipo de conta já existe<br>"
      end if
   else
      msg=msg&" Tipo de conta.<br>"
   end if
   if msg="" then
      conn.execute("update tipo_conta set descricao='"&descricao&"' where codigo="&codigo)%>
      <script>
    ...
    <td>
      ...
      <table border="0" align=left width=380 cellspacing="0" cellpadding="0">
        <tr>
          <td>
            <br>
            <span span class="estilo">&nbsp;&nbsp;<b>Para alterar um tipo de conta siga os passos descrito abaixo.<br><br></b>
      ...
      <table border="0" width=376 align=right>
        <tr>
          <td span class="estilo" width=100%>
            <img src="../img/r_14.gif">&nbsp;ALTERAR TIPO DE CONTA<br><br>
          </td>
