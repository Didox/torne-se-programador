...
if request("flag") <> 1 and reload = "" then
   set registro_midia=conn.execute("select * from registro_midia where codigo="&codigo)
   imovel=registro_midia("cod_imovel")
   unidade=registro_midia("cod_unidade_metrica")
   veiculo=registro_midia("cod_veiculo_comunicacao")
   quantidade=registro_midia("quantidade")
else
   ...
   set verifica=conn.execute("select * from registro_midia where quantidade="&quantidade&" and cod_imovel="&imovel&" and cod_veiculo_comunicacao="&veiculo&" and cod_unidade_metrica="&unidade&" and status=0 and codigo="&codigo)
   if verifica.eof then
     if msg="" and reload = "" then
       conn.execute("update registro_midia set cod_unidade_metrica="&unidade&", cod_veiculo_comunicacao="&veiculo&", quantidade="&quantidade&" where codigo="&codigo)%>
       <script>
     ...
     msg=msg&" Registro já casdatrdo.<br>"
   end if
end if%>
<html>
<head>
...
<form method=post action="<%=request.servervariables("SCRIPT_NAME")%>?flag=1&pagina=<%=pagina%>&codigo=<%=codigo%>&imovel=<%=imovel%>" name="form1">
    ...
    <td>
      ...
      <table border="0" align=left width=380 cellspacing="0" cellpadding="0">
        <tr>
          <td>
            <br>
            <span span class="estilo">&nbsp;&nbsp;<b>Para alterar um bairro siga os passos descrito abaixo.<br><br></b>
      ...
      <table border="0" width=376 align=right>
        <tr>
          <td span class="estilo" width=100%>
            <img src="../img/r_14.gif">&nbsp;ALTERAR REGISTRO DE MÍDIA<br><br>
          </td>
          ...
          <td>
            <select name="unidade" style="width:140;background=#E8E8E8;" onchange="document.form1.reload.value='1';document.form1.submit();">
             ...
             <%do while not cvec.eof %>
              <%if cstr(cvec("codigo"))=cstr(unidade) then%>
        ...
        <tr>
          ...
          <%else%>
            <select name="veiculo" style="width:140;background=#E8E8E8;">
             ...
             <%do while not  cvec.eof %>
              <%if cstr( cvec("codigo"))=cstr(veiculo) then%>
                 <option value="<%= cvec("codigo")%>" selected><%= cvec("descricao")%>
              <%else%>
                 <option value="<%= cvec("codigo")%>"><%= cvec("descricao")%>
              <%end if%>
