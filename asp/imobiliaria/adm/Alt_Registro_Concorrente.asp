...
if request("flag") <> 1 and reload = "" then
   set registro_concorrente=conn.execute("select * from registro_concorrente where codigo="&codigo)
   imovel=registro_concorrente("cod_imovel")
   concorrente=registro_concorrente("cod_concorrente")
else
   ...
   set verifica=conn.execute("select * from registro_concorrente where cod_concorrente="&concorrente&" and codigo<>"&codigo)
   if verifica.eof then
     if msg="" and reload = "" then
       conn.execute("update registro_concorrente set cod_concorrente="&concorrente&" where codigo="&codigo)%>
       <script>
     ...
     msg=msg&" Concorrente já casdatrdo.<br>"
   end if
end if%>
<html>
<head>
    ...
    <td>
      ...
      <table border="0" align=left width=380 cellspacing="0" cellpadding="0">
        <tr>
          <td>
            <br>
            <span span class="estilo">&nbsp;&nbsp;<b>Para alterar um registro do concorrente siga os passos descrito abaixo.<br><br></b>
      ...
      <table border="0" width=376 align=right>
        <tr>
          <td span class="estilo" width=100%>
            <img src="../img/r_14.gif">&nbsp;ALTERAR REGISTRO DE CONCORRENTE<br><br>
          </td>
          ...
          <td>
            <select name="concorrente" style="width:140;background=#E8E8E8;">
             ...
             <%do while not  cvec.eof %>
              <%if cstr( cvec("codigo"))=cstr(concorrente) then%>
