...
if request("flag") <> 1 then
   set comodo=conn.execute("select * from tipo_comodo where codigo="&codigo)
   descricao=replace(trim(ucase(comodo("descricao"))),"'","")
else
   descricao=trim(ucase(request("descricao")))
   if descricao<>"" then
      set verifica_descricao=conn.execute("select * from tipo_comodo where descricao='"&descricao&"' and codigo<>"&codigo)
      if not verifica_descricao.eof then
         msg=msg&" Comodo já existe<br>"
      end if
   else
      msg=msg&" Comodo.<br>"
   end if
   if msg="" then
      conn.execute("update tipo_comodo set descricao='"&descricao&"' where codigo="&codigo)%>
      <script>
    ...
    <td>
      ...
      <table border="0" align=left width=380 cellspacing="0" cellpadding="0">
        <tr>
          <td>
            <br>
            <span span class="estilo">&nbsp;&nbsp;<b>Para alterar o tipo de comodo siga os passos descrito abaixo.<br><br></b>
