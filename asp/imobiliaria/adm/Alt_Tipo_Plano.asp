...
if request("flag") <> 1 then
   set motivo=conn.execute("select * from tipo_conta where tipo=0 and codigo="&codigo)
   descricao=replace(trim(ucase(motivo("descricao"))),"'","")
   meses=trim(motivo(("meses")))
else
   descricao=trim(ucase(request("descricao")))
   meses=trim(request(("meses")))
   if descricao<>"" then
      set verifica_descricao=conn.execute("select * from tipo_conta where descricao='"&descricao&"' and codigo<>"&codigo)
      if not verifica_descricao.eof then
         msg=msg&" Motivo já existe<br>"
      end if
   else
      msg=msg&" Motivo.<br>"
   end if
   if not isnumeric(meses) then
      msg=msg&" Meses.<br>"
   end if
   if msg="" then
      conn.execute("update tipo_conta set descricao='"&descricao&"', meses="&meses&" where codigo="&codigo)%>
      <script>
    ...
    <td>
      ...
      <table border="0" align=left width=380 cellspacing="0" cellpadding="0">
        <tr>
          <td>
            <br>
            <span span class="estilo">&nbsp;&nbsp;<b>Para alterar o tipo de plano siga os passos descrito abaixo.<br><br></b>
      ...
      <table border="0" width=376 align=right>
        <tr>
          <td span class="estilo" width=100%>
            <img src="../img/r_14.gif">&nbsp;ALTERAR TIPO DE plano<br><br>
          </td>
        ...
        <tr>
          <td>
            <img src="../img/obrigacao.gif">&nbsp;<span class="estilo">Meses:
          </td>
        </tr>
        <tr>
          <td>
            <input type="text" name="meses" value="<%=meses%>" maxlength=3 label="meses" style="width:40; height:17; background=#E8E8E8;" onkeypress="Numero()">
          </td>
        </tr>
        <tr>
          <td align=center>
