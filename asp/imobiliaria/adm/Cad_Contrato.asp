...
if request("flag") = 1 then
   ...
   if msg="" and reload="" then
       set itens = conn.execute("insert into contrato (cod_cliente,numero,cod_imovel,cod_fiador1,cod_fiador2,tipo_servico,cod_corretor,data_inicio,data_vencimento,valor,multa,taxa,cod_indice,reajuste,comissao, data_inclusao,cod_chave_imovel) values ("&cliente&","&numero&","&imovel&","&fiador1&","&fiador2&","&servicos&","&corretor&",convert(datetime , '"&inicio&"' , 103), convert(datetime , '"&vencimento&"' , 103),"&replace(replace(csng(valor),".",""),",",".")&","&multa&","&taxa&","&indice&",convert(datetime,'"&reajuste&"',103),"&replace(replace(csng(comissao),".",""),",",".")&",convert(datetime,'"&date()&"',103),"&chave&")")
       set update_chave = conn.execute("update registro_chave set status=1 where codigo="&chave)       
...
<%if imovel<>"" and Imovel<>"0" then%>
 ...
 <table cellspacing=0 cellpadding=0>
    ...
    <tr>
      <td>
        <select name="chave" style="background=#E8E8E8;">
          <option value=""></option>
            <%set chaves=conn.execute("select cod_quadro , registro_chave.codigo as codigo from (registro_chave inner join imoveis on imoveis.codigo=registro_chave.cod_imovel) where registro_chave.status=0 and registro_chave.cod_imovel="&imovel)%>
    ...
    <tr>
       ...
       <td>
         <input type="text" name="comissao" value="<%=comissao%>" label="reajuste" style="width:50; height:17;background:#E8E8E8;text-align: RIGHT;" maxlength="18" onkeypress="Numero();" onkeyup="Moeda(this);">%
       </td>
    </tr>
    <tr>
        <td colspan=2>
            <table cellspacing=0 cellpadding=0>
              ...
              <tr>
                ...
                <td>
                  <input type="text" name="valor_plano" value="" label="valor_plano" style="width:60; height:17;background:#E8E8E8;text-align: RIGHT;" maxlength="18" onkeypress="Numero();" onkeyup="Moeda(this);">
                </td>
                <td align=center>
