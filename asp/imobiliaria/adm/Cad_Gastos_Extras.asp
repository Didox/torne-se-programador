...
if request("flag") = 1 then
   tipo_despesa  =trim(ucase(request("tipo_despesa")))
   data=request("data")
   setor=request("setor")
   ...
   if tipo_despesa="" then
      msg=msg&" Tipo da Despesa.<br>"
   end if
   if valor="" then
      msg=msg&" Valor.<br>"
   end if
   if not data_valida(data_emissao,0) then   
     msg=msg&" Data.<br>"
   end if

   if msg="" then
     conn.execute("insert into Gastos_extra (cod_imovel,cod_despesa,data_inclusao,valor) values ("&imovel&",'"&tipo_despesa&"',convert(datetime,'"&date()&"',103),"&replace(replace(csng(valor),".",""),",",".")&")")%>
      <script>
      ...
      <table border="0" width=376 align=right>
        <tr>
          <td span class="estilo" width=100%>
            <img src="../img/r_11.gif">&nbsp;CADASTRAR CONTAS<br><br>
          </td>
        ...
        <tr>
          <td>
             <%set imovel=conn.execute("select * from imoveis where status=0 and codigo="&imovel&" ORDER BY descricao")%>
             <%=imovel("descricao")%>
        ...
        <tr>
          <td>
            <select name="setor" style="width:140;background=#E8E8E8;">
              ...
              <%if cstr(cbai("codigo"))=setor then%>
        ...
        <tr>
          <td>
            <table cellspacing=0 cellpadding=0>
              ...
              <tr>
                <td>
                  <input type="text" name="descricao" value="<%=descricao%>"  label="descricao" style="width:150; height:17; background=#E8E8E8;">
                </td>
              </tr>
              <tr>
                <td>
