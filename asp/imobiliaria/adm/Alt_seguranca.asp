...
if request("flag") <> 1 then
   set seguranca=conn.execute("select * from seguranca where codigo="&codigo)
   descricao=replace(trim(ucase(seguranca("descricao"))),"'","")
else
   descricao=trim(ucase(request("descricao")))
   if descricao<>"" then
      set verifica_descricao=conn.execute("select * from seguranca where descricao='"&descricao&"' and codigo<>"&codigo)
      if not verifica_descricao.eof then
         msg=msg&" seguranca já existe<br>"
      end if
   else
      msg=msg&" seguranca.<br>"
   end if
   if msg="" then
      conn.execute("update seguranca set descricao='"&descricao&"' where codigo="&codigo)%>
      <script>
    ...
    <td>
      ...
      <table border="0" align=left width=380 cellspacing="0" cellpadding="0">
        <tr>
          <td>
            <br>
            <span span class="estilo">&nbsp;&nbsp;<b>Para alterar uma área de segurança siga os passos descrito abaixo.<br><br></b>
      ...
      <table border="0" width=376 align=right>
        <tr>
          <td span class="estilo" width=100%>
            <img src="../img/r_14.gif">&nbsp;ALTERAR ÁREA DE SEGURANÇA<br><br>
          </td>
