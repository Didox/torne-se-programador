...
if request("flag") <> 1 then
   set metragem=conn.execute("select * from metragem where codigo="&codigo)
   descricao=replace(trim(ucase(metragem("descricao"))),"'","")
else
   descricao=trim(ucase(request("descricao")))
   if descricao<>"" then
      set verifica_descricao=conn.execute("select * from metragem where descricao='"&descricao&"' and codigo<>"&codigo)
      if not verifica_descricao.eof then
         msg=msg&" Metragem já existe<br>"
      end if
   else
      msg=msg&" Metragem.<br>"
   end if
   if msg="" then
      conn.execute("update metragem set descricao='"&descricao&"' where codigo="&codigo)%>
      <script>
    ...
    <td>
      ...
      <table border="0" align=left width=380 cellspacing="0" cellpadding="0">
        <tr>
          <td>
            <br>
            <span span class="estilo">&nbsp;&nbsp;<b>Para alterar uma metragem siga os passos descrito abaixo.<br><br></b>
      ...
      <table border="0" width=376 align=right>
        <tr>
          <td span class="estilo" width=100%>
            <img src="../img/r_14.gif">&nbsp;ALTERAR METRAGEM<br><br>
          </td>
        ...
        <tr>
          <td>
            <input type="text" name="descricao" value="<%=descricao%>" maxlength=50 onkeypress="Numero()" label="Serviço" style="width:290; height:17; background=#E8E8E8;">
          </td>
        </tr>
        <tr>
          <td align=center>
