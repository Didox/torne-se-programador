...
if request("flag") <> 1 then
   set estagios_obra=conn.execute("select * from estagios_obra where codigo="&codigo)
   descricao=replace(trim(ucase(estagios_obra("descricao"))),"'","")
else
   descricao=trim(ucase(request("descricao")))
   if descricao<>"" then
      set verifica_descricao=conn.execute("select * from estagios_obra where descricao='"&descricao&"' and codigo<>"&codigo)
      if not verifica_descricao.eof then
         msg=msg&" Estágio de obra já existe<br>"
      end if
   else
      msg=msg&" Estágio da Obra.<br>"
   end if
   if msg="" then
      conn.execute("update estagios_obra set descricao='"&descricao&"' where codigo="&codigo)%>
      <script>
    ...
    <td>
      ...
      <table border="0" align=left width=380 cellspacing="0" cellpadding="0">
        <tr>
          <td>
            <br>
            <span span class="estilo">&nbsp;&nbsp;<b>Para alterar um estágio siga os passos descrito abaixo.<br><br></b>
      ...
      <table border="0" width=376 align=right>
        <tr>
          <td span class="estilo" width=100%>
            <img src="../img/r_14.gif">&nbsp;ALTERAR ESTÁGIO DA OBRA<br><br>
          </td>
