...
if request("flag") <> 1 then
   set estado_civil=conn.execute("select * from tipo_estado_civil where codigo="&codigo)
   descricao=replace(trim(ucase(estado_civil("descricao"))),"'","")
else
   descricao=trim(ucase(request("descricao")))
   if descricao<>"" then
      set verifica_descricao=conn.execute("select * from tipo_estado_civil where descricao='"&descricao&"' and codigo<>"&codigo)
      if not verifica_descricao.eof then
         msg=msg&" Estado Civil já existe<br>"
      end if
   else
      msg=msg&" Estado Civil.<br>"
   end if
   if msg="" then
      conn.execute("update tipo_estado_civil set descricao='"&descricao&"' where codigo="&codigo)%>
      <script>
    ...
    <td>
      ...
      <table border="0" align=left width=380 cellspacing="0" cellpadding="0">
        <tr>
          <td>
            <br>
            <span span class="estilo">&nbsp;&nbsp;<b>Para alterar um estado civil siga os passos descrito abaixo.<br><br></b>
      ...
      <table border="0" width=376 align=right>
        <tr>
          <td span class="estilo" width=100%>
            <img src="../img/r_14.gif">&nbsp;ALTERAR ESTADO CIVIL<br><br>
          </td>
