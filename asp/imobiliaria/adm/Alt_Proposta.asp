...
if request("flag") <> 1 then
   set propostas=conn.execute("select * from propostas where codigo="&codigo)
   descricao=replace(trim(ucase(propostas("descricao"))),"'","")
   texto=replace(propostas("texto"),"<br>",chr(13))
else
   descricao=trim(ucase(request("descricao")))
   texto=replace(replace(trim(ucase(request("texto"))),"'",""),chr(13),"<br>")
   if descricao<>"" then
      set verifica_descricao=conn.execute("select * from propostas where descricao='"&descricao&"' and codigo<>"&codigo)
      if not verifica_descricao.eof then
         msg=msg&" Propostas já existe<br>"
      end if
   else
   ...
   if msg="" then
      conn.execute("update propostas set descricao='"&descricao&"', texto='"&texto&"' where codigo="&codigo)%>
      <script>
