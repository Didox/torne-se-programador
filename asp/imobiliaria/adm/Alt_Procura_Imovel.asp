...
if request("flag") <> 1 then
   set Procura_imovel=conn.execute("select * from Procura_imovel where codigo="&codigo)
   nome=replace(trim(ucase(Procura_imovel("nome"))),"'","")
   ddd=trim(ucase(Procura_imovel("ddd")))
   fone=trim(ucase(Procura_imovel("fone")))
   email=replace(trim(ucase(Procura_imovel("email"))),"'","")
else
   ...
   if nome<>"" then
      set verifica_nome=conn.execute("select * from Procura_Imovel where nome='"&nome&"' and codigo<>"&codigo)
      if not verifica_nome.eof then
         msg=msg&" Procura de imovel já existe<br>"
      end if
   else
      msg=msg&" Procura Imovel.<br>"
   end if
   if msg="" then
      conn.execute("update Procura_Imovel set nome='"&nome&"' , ddd="&ddd&" , fone="&fone&" , email='"&email&"' where codigo="&codigo)%>
    <script>
    ...
    <td>
      ...
      <table border="0" align=left width=380 cellspacing="0" cellpadding="0">
        <tr>
          <td>
            <br>
            <span span class="estilo">&nbsp;&nbsp;<b>Para alterar a procura de imovel siga os passos descrito abaixo.<br><br></b>
      ...
      <table border="0" width=376 align=right>
        <tr>
          <td span class="estilo" width=100%>
            <img src="../img/r_8.gif">&nbsp;CADASTRAR PROCURA DE IMOVEL 
          </td>
        ...
        <tr>
          <td>
           <img src="../img/obrigacao.gif"> <span class="estilo">E-mails:
          </td>
        </tr>
        <tr>
          <td>
