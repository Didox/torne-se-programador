...
itens.Open "select * from equipe where status=0 "&sql&" order by descricao "&ordem2,conn%>
<html>
<head>
<title></title>
  ...
  <tr>
    <td class="navdroplist">
      ...
      <table width=100% align="center" bgcolor="#f7f7f7" border=0>
        <tr>
          <td class="navdroplist" align=center><B>RELATÓRIO EQUIPE(S)</B></td>
        </tR>
      </table>
      <br>
      ...
      <table width=100% align="center" bgcolor="#FFFFFF" border=0>
        ...
        <%else%>
         <tr>
           ...
           <td class="navdroplist"><b>SUPERVISOR:</b></td>
         </tr>
       <tr><td class="navdroplist"><br></td></tr>
       <%do while not itens.eof%>
       <tr>
         <td class="navdroplist"><%=itens("descricao")%></td>
         <%set supervisor = conn.execute("select nome from corretor where codigo="&itens("cod_supervisor"))%>
         <%if itens("cod_supervisor")<>"0" then%>
           <td class="navdroplist"><%=supervisor("nome")%></td>
         <%else%>
           <td class="navdroplist">SUPERVISOR NÃO CADASTRADO</td>
         <%end if%>
       </tr>
      <%itens.MoveNext
       loop%>
