...
if request("flag") <> 1 then
  ...
  corretor=contas("cod_funcionario")
  ...
  descricao=replace(trim(ucase(request("descricao"))),"'","")
   ...
   if msg="" then
    conn.execute("update contas set tipo="&tipo&", data=convert(datetime,'"&data&"',103), cod_funcionario="&corretor&", cod_tipo_conta="&tipo_conta&", valor='"&replace(replace(csng(valor),".",""),",",".")&"', descricao='"&descricao&"' where codigo="&codigo)%>
     <script>
    ...
    <td>
      ...
      <table border="0" align=left width=380 cellspacing="0" cellpadding="0">
        <tr>
          <td>
            <br>
            <span span class="estilo">&nbsp;&nbsp;<b>Para alterar comissão siga os passos descrito abaixo.<br><br></b>
      ...
      <table border="0" width=376 align=right>
       <tr>
          <td span class="estilo" width=100%>
            <img src="../img/r_14.gif">&nbsp;ALTERAR COMISSÃO
          </td>
       ...
       <tr>
         <td>
            <%if cstr(tipo)="" or cstr(tipo)="0" then%>
               <input type="radio" name="tipo" value="0" checked><span class="estilo">Comissão
               <input type="radio" name="tipo" value="1"><span class="estilo">Estorno
            <%else%>
               <input type="radio" name="tipo" value="0"><span class="estilo">Comissão
               <input type="radio" name="tipo" value="1" checked><span class="estilo">Estorno
            <%end if%>
         </td>
       </tr>
       <tr>
       ...
       <tr>
         <td>
           <%set contrato = conn.execute("select * from contrato where codigo="&codigo2)%>
       ...
       <tr>
         <td>
           <input type="hidden" name="cod_imovel" value="<%=contrato("codigo")%>">
