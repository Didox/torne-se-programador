...
if request("flag") = 1 then
   ...
   if descricao<>"" then
      set verifica_descricao=conn.execute("select * from propostas where descricao='"&descricao&"'")
      if not verifica_descricao.eof then
         msg=msg&" Proposta já existe<br>"
      end if
   else
      msg=msg&" Proposta.<br>"
   end if
   ...
   if msg="" then
      conn.execute("insert into propostas (descricao, texto, data_inclusao) values ('"&descricao&"', '"&replace(texto,chr(13),"<br>")&"', convert(datetime,'"&date()&"',103))")%>
      <script>
