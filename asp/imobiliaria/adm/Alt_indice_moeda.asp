...
if request("flag") <> 1 then
   set indice = conn.execute("select * from indice_moeda where codigo = "&codigo2)
   valor=trim(formatnumber(csng(indice("valor")),2))
   data=data_completa(indice("data_inclusao"))
else
   ...
   if msg = "" then
   set valida_indice = conn.execute("select * from indice_moeda where cod_moeda="&codigo&" and data_inclusao=convert(datetime,'"&data&"',103) and valor = "&replace(replace(csng(valor),".",""),",",".")&" and codigo ="& codigo2&" and cod_moeda="&codigo)
   if valida_indice.eof then
      if msg="" then
        conn.execute("update indice_moeda set valor = "&replace(replace(csng(valor),".",""),",",".")&", data_inclusao = convert(datetime,'"&data&"',103) where codigo="&codigo2)%>
        <script>
      ...
      <table border="0" width=376 align=right>
        <tr>
          <td span class="estilo" width=100%>
            <img src="../img/r_14.gif">&nbsp;ALTERAR ÍNDICE DE MOEDA
          </td>
        ...
        <tr>
          <td>
            <input type="text" name="data" value="<%=data%>"  maxlength=10 label="data" style="width:80; height:17; background=#E8E8E8;"onkeypress="Numero();DATA(this);">
          </td>
        </tr>
        <tr>
          <td>
