...
if request("flag") = 1 then
   ...
   if descricao<>"" then
      set verifica_descricao=conn.execute("select * from Situacao where descricao='"&descricao&"'")
      if not verifica_descricao.eof then
         msg=msg&" Localização já existe.<br>"
      end if
   else
      msg=msg&" Descrição.<br>"
   end if
   if msg="" then
      conn.execute("insert into Situacao (descricao,data_inclusao) values ('"&descricao&"',convert(datetime,'"&date()&"',103))")%>
      <script>
    ...
    <td>
      ...
      <table border="0" align=left width=380 cellspacing="0" cellpadding="0">
        <tr>
          <td>
            <br>
            <span span class="estilo">&nbsp;&nbsp;<b>Para cadastrar uma nova situação siga os passos descrito abaixo.<br><br></b>
      ...
      <table border="0" width=376 align=right>
        <tr>
          <td span class="estilo" width=100%>
            <img src="../img/r_14.gif">&nbsp;CADASTRAR NOVA SITUAÇÃO<br><br>
          </td>
