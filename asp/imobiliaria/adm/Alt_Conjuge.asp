...
if request("flag") <> 1 then
   set conjuge=conn.execute("select * from conjuge where codigo="&codigo)
   cliente=conjuge("cod_cliente")
   logradouro=conjuge("endereco")
   numero=conjuge("numero")
   compl=conjuge("complemento")
   bairro=conjuge("bairro")
   cidade=conjuge("cidade")
   estado=conjuge("estado")
   cep=conjuge("cep")
   nome=conjuge("nome")
   cpf=conjuge("cpf")
   identidade=conjuge("identidade")
   ddd=conjuge("ddd_fone")
   fone=conjuge("fone")
   dddc=conjuge("ddd_cel")
   celular=conjuge("celular")
   profissao=conjuge("profissao")
   renda=trim(formatnumber(csng(conjuge("renda")),2))
   email=conjuge("email")
   data_nascimento=data_completa(conjuge("data_nascimento"))
   data_emissao   =data_completa(conjuge("data_emissao"))
   orgao_emissor  =conjuge("orgao_emissor")
else
   ...
   ddd=trim(ucase(request("ddd_fone")))
   ...
   if cpf<>"" then
      ...
      if not CalculaCPF(cpf) then
         ...
         set verifica_cpf=conn.execute("select * from conjuge where cpf='"&cpf&"' and codigo<>"&codigo)
   ...
   if msg="" then
      cpf=replace(cpf,".","")
      cpf=replace(cpf,"-","")
      conn.execute("update conjuge set nome = '"&nome&"', endereco = '"&logradouro&"', numero = '"&numero&"', complemento = '"&compl&"', bairro = '"&bairro&"', cidade = '"&cidade&"', estado = '"&estado&"', cep = '"&cep&"', ddd_fone = '"&ddd&"', fone = '"&fone&"', ddd_cel = '"&dddc&"', celular = '"&celular&"', cpf = '"&cpf&"', identidade = '"&identidade&"', email = '"&email&"', renda = "&replace(replace(csng(renda),".",""),",",".")&", profissao = '"&profissao&"', data_inclusao = convert(datetime,'"&date()&"',103),data_nascimento=convert(datetime,'"&data_nascimento&"',103),data_emissao=convert(datetime,'"&data_emissao&"',103),orgao_emissor='"&orgao_emissor&"' where codigo="&codigo)%>
      <script>
...
<form method=post action="<%=request.servervariables("SCRIPT_NAME")%>?flag=1&pagina=<%=pagina%>&cliente=<%=cliente%>&codigo=<%=codigo%>" name="form1">
    ...
    <td>
      ...
      <table border="0" align=left width=380 cellspacing="0" cellpadding="0">
        <tr>
          <td>
            <br>
            <span span class="estilo">&nbsp;&nbsp;<b>Para alterar o conjugê siga os passos descrito abaixo.<br><br></b>
      ...
      <table border="0" width=376 align=right>
        <tr>
          <td span class="estilo" width=100%>
            <img src="../img/r_8.gif">&nbsp;ALTERAR CONJUGÊ
          </td>
