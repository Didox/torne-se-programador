...
if request("flag") <> 1 then
   set Forma_pagamento=conn.execute("select * from Forma_pagamento where codigo="&codigo)
   descricao=replace(trim(ucase(Forma_pagamento("descricao"))),"'","")
else
   descricao=trim(ucase(request("descricao")))
   if descricao<>"" then
      set verifica_descricao=conn.execute("select * from Forma_pagamento where descricao='"&descricao&"' and codigo<>"&codigo)
      if not verifica_descricao.eof then
         msg=msg&" Forma de Pagamento já existe<br>"
      end if
   else
      msg=msg&" Forma de Pagamento.<br>"
   end if
   if msg="" then
      conn.execute("update Forma_pagamento set descricao='"&descricao&"' where codigo="&codigo)%>
      <script>
    ...
    <td>
      ...
      <table border="0" align=left width=380 cellspacing="0" cellpadding="0">
        <tr>
          <td>
            <br>
            <span class="estilo">&nbsp;&nbsp;<b>Para alterar forma de pagamento siga os passos descrito abaixo.<br><br></b>
      ...
      <table border="0" width=376 align=right>
        <tr>
          <td span class="estilo" width=100%>
            <img src="../img/r_14.gif">&nbsp;ALTERAR FORMA DE PAGAMENTO<br><br>
          </td>
