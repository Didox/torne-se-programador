...
set acompanhamentos=conn.execute("select * from acompanhamento where codigo="&codigo2)
if request("flag") <> 1 then
   descricao=acompanhamentos("descricao")
   tipo=trim(ucase(acompanhamentos("acompanhamento")))
   corretor=trim(ucase(acompanhamentos("cod_corretor")))
   imovel=trim(ucase(acompanhamentos("cod_imovel")))
   data=trim(ucase(acompanhamentos("data")))
   acomp=request(acompanhamentos("cod_acompanhamento"))
else
   ...
   t=request("t")
   if descricao<>"" then
      set verifica_descricao=conn.execute("select * from acompanhamento where descricao='"&descricao&"' and codigo<>"&codigo2)
      if not verifica_descricao.eof then
         msg=msg&" Acompanhamento já existe<br>"
      end if
   else
      msg=msg&" acompanhamento.<br>"
   end if
   if acomp = "" then
     acomp="0"
   else
   ...
   if msg="" and reload = "" then
     conn.execute("update acompanhamento set cod_imovel="&imovel&" , cod_corretor="&corretor&" , cod_acompanhamento="&acomp&" , data=convert(datetime,'"&t&"',103) , descricao='"&descricao&"', acompanhamento="&tipo&" where codigo="&codigo2)%>
      <script>
...
<form method=post action="<%=request.servervariables("SCRIPT_NAME")%>?flag=1&tempo=<%=t%>&tipo=<%=tipo%>&pagina=<%=pagina%>&acomp=<%=acomp%>&codigo=<%=codigo%>&codigo2=<%=codigo2%>&imovel=<%=imovel%>&corretor=<%=corretor%>" name="form1">
<table border="0" width="777" cellspacing="0" cellpadding="0">
  <tr>
    <td>
      ...
      <table border="0" align=left width=380 cellspacing="0" cellpadding="0">
        <tr>
          <td>
            <br>
            <span span class="estilo">&nbsp;&nbsp;<b>Para alterar um acompanhamento siga os passos descrito abaixo.<br><br></b>
      ...
      <table border="0" width=376 align=right>
        <tr>
          <td span class="estilo" width=100%>
            <img src="../img/r_14.gif">&nbsp;ALTERAR ACOMPANHAMENTO 
          </td>
          <td bgcolor="#FFFFFF">&nbsp;&nbsp;</td>
        </tr>
        <tr>
         <td>
           <input type=checkbox name=acomp <%if cstr(acomp)="1" then%> checked <%end if%> value=1><span class="estilo">Acompanhamento efetuado:
         </td>
       </tr>
        <tr>
          <td>
    ...
    <table cellspacing=0 cellpadding=0>
       ...
       <tr>
         <td>
            <%set imovel=conn.execute("select * from imoveis where codigo="&imovel&" ORDER BY descricao")%>
            <input type="hidden" name="imovel" value=<%=imovel("codigo")%>>
            <b><%=imovel("descricao")%></b>
         </td>
        <td>
            <%set corretor=conn.execute("select * from corretor where codigo="&corretor&" ORDER BY nome")%>
            <input type="hidden" name="corretor" value=<%=corretor("codigo")%>>
            <b>&nbsp;&nbsp;&nbsp;&nbsp;<%=corretor("nome")%></b>
         </td>
        </tr>
      </table>
 ...
 <table cellspacing=0 cellpadding=0>
    ...
    <tr>
      <td>
        ...
        <select name="tipo" style="background=#E8E8E8;" onchange="document.form1.reload.value='1';;document.form1.submit()">
          ...
          <%if tipo = "" then%>
            <%tipo=cbai("codigo2")%>
          <%end if%>
          <%if cstr(cbai("codigo"))=cstr(tipo) then%>
      ...
      <input type="text" value="<%=date()+tempo("tempo")%>" name="t">
    </tr>
   </table>  
     <tr> 
       <td align=center>
