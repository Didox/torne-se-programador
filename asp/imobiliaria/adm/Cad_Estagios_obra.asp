...
if request("flag") = 1 then
   ...
   if descricao<>"" then
      set verifica_descricao=conn.execute("select * from estagios_obra where descricao='"&descricao&"'")
      if not verifica_descricao.eof then
         msg=msg&" Tipo de imoveis já existe<br>"
      end if
   else
      msg=msg&" Estágio da Obra.<br>"
   end if
   if msg="" then
      conn.execute("insert into estagios_obra (descricao) values ('"&descricao&"')")%>
      <script>
      ...
      <table border="0" width=376 align=right>
        <tr>
          <td span class="estilo" width=100%>
            <img src="../img/r_14.gif">&nbsp;CADASTRAR NOVO ESTÁGIO DE OBRA<br><br>
          </td>
