...
if request("flag") = 1 then
   ...
   if descricao<>"" then
      set verifica_descricao=conn.execute("select * from opcao where descricao='"&descricao&"'")
      if not verifica_descricao.eof then
         msg=msg&" Opcão já existe<br>"
      end if
   else
      msg=msg&" Opção.<br>"
   end if
   if msg="" then
      conn.execute("insert into opcao (descricao ) values ('"&descricao&"')")%>
      <script>
    ...
    <td>
      ...
      <table border="0" align=left width=380 cellspacing="0" cellpadding="0">
        <tr>
          <td>
            <br>
            <span span class="estilo">&nbsp;&nbsp;<b>Para cadastrar uma nova opção siga os passos descrito abaixo.<br><br></b>
      ...
      <table border="0" width=376 align=right>
        <tr>
          <td span class="estilo" width=100%>
            <img src="../img/r_14.gif">&nbsp;CADASTRAR NOVA OPÇÃO<br><br>
          </td>
