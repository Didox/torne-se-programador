...
if request("flag") <> 1 then
   set Visitas=conn.execute("select * from Visita where codigo="&codigo)
   hora        =replace(trim(ucase(visitas("hora_visita"))),"'","")
   corretor    =replace(trim(ucase(visitas("cod_corretor"))),"'","")
   cod_imovel  =trim(ucase(visitas("cod_imovel")))
   data_visita =replace(trim(ucase(visitas("data_visita"))),"'","")
   nome        =replace(trim(ucase(visitas("nome"))),"'","")
   endereco    =replace(trim(ucase(visitas("endereco"))),"'","")
   numero      =replace(trim(ucase(visitas("numero"))),"'","")
   complemento =replace(trim(ucase(visitas("complemento"))),"'","")
   bairro      =replace(trim(ucase(visitas("bairro"))),"'","")
   cidade      =replace(trim(ucase(visitas("cidade"))),"'","")
   estado      =replace(trim(ucase(visitas("estado"))),"'","")
   cep         =replace(trim(ucase(visitas("cep"))),"'","")
   endereco    =replace(trim(ucase(visitas("endereco"))),"'","")
   ddd         =replace(trim(ucase(visitas("ddd"))),"'","")
   fone        =replace(trim(ucase(visitas("fone"))),"'","")
   email       =replace(trim(ucase(visitas("email"))),"'","")
   servicos    =replace(trim(ucase(visitas("cod_servico"))),"'","")
   visita      =trim(ucase(visitas("visita")))  
   classificacao=visitas("cod_classificacao_interessado")
else
   ...
   if msg="" and reload=""then
     set procura = conn.execute("select codigo from visita where hora_visita='"&hora&"' and data_visita=convert(datetime,'"&data_visita&"',103) and cod_corretor='"&corretor&"' and nome='"&nome&"' and codigo<>"&codigo)
     if not procura.eof then
      msg=msg&" Já existe visita agendada para este horário."
     end if
   end if 
   if msg="" and reload="" then
       conn.execute("update Visita set hora_visita='"&hora&"',cod_corretor="&corretor&",cod_imovel="&cod_imovel&",data_visita=convert(datetime,'"&data_visita&"',103),nome='"&nome&"',email='"&email&"',ddd="&ddd&",fone='"&fone&"',cod_servico="&servicos&",visita="&visita&",endereco='"&endereco&"',numero='"&numero&"',complemento='"&complemento&"',bairro='"&bairro&"',cidade='"&cidade&"',estado='"&estado&"',cep='"&cep&"',Cod_classificacao_interessado="&classificacao&" where codigo="&codigo)%>
      <script>
  ...
  <tr>
    <td>
      ...
      <table border="0" width=376 align=right>
        ...
        <tr>
          <td>
            <input type=checkbox name="visita" <%if cstr(visita)="1" then%> checked <%end if%>value=1><span class="estilo">Visita efetuada:
          </td>
        </tr>
      <tr>
         <td>
           <img src="../img/obrigacao.gif">&nbsp;<span class="estilo">Classificação do Interressado:
         </td>
       </tr>    
       <tr>
        <td>
         <select name="classificacao" style="background=#E8E8E8;">
          <option value= ""></option>
          <%set cbai=conn.execute("select * from classificacao_interessado ORDER BY descricao")%>
    ...
    <tr>
	  ...
	  <select name="cod_imovel" style="background=#E8E8E8;" onchange="document.form1.reload.value='1';document.form1.submit()">
	   ...
	   <%do while not imoveis.eof %>
	      <%if cstr(imoveis("codigo"))= cstr(cod_imovel) then%>
