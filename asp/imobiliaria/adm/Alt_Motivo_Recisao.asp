...
if request("flag") <> 1 then
   set motivo=conn.execute("select * from tipo_motivo where codigo="&codigo)
   descricao=replace(trim(ucase(motivo("descricao"))),"'","")
else
   descricao=trim(ucase(request("descricao")))
   if descricao<>"" then
      set verifica_descricao=conn.execute("select * from tipo_motivo where descricao='"&descricao&"' and codigo<>"&codigo)
   ...
   if msg="" then
      conn.execute("update tipo_motivo set descricao='"&descricao&"' where codigo="&codigo)%>
      <script>
    ...
    <td>
      ...
      <table border="0" align=left width=380 cellspacing="0" cellpadding="0">
        <tr>
          <td>
            <br>
            <span span class="estilo">&nbsp;&nbsp;<b>Para alterar um motivo da recisão siga os passos descrito abaixo.<br><br></b>
      ...
      <table border="0" width=376 align=right>
        <tr>
          <td span class="estilo" width=100%>
            <img src="../img/r_14.gif">&nbsp;ALTERAR MOTIVO DA RECISÃO<br><br>
          </td>
