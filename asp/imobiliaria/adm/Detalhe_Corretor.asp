  ...
  <tr>
    <td>
      <!--#INCLUDE FILE ="Main_Topo.asp"-->
          <%set exibir  = conn.execute("select * from corretor where codigo="&codigo)%>
          ...
          <table width=100% border=0 bgcolor=#E8E8E8 cellspacing="0" cellpadding="0">
            <tr> 
              ...
              <td class="tb">
                   <B>DETALHE CORRETOR</B>
              </td>
              <td class="tb" align=right><a href="Imprimir_contrato_corretor.asp?codigo=<%=codigo%>&pagina=Detalhe_corretor"><img alt="Imprimir Contratos" src="../img/icone-imprimir.gif" border=0></a></td>
            </tr>
          </table>
          ...
          <table width=100% border=0 cellspacing="0" cellpadding="0">
            <tr>
              <td class="tb" width=60><b>&nbsp;EQUIPE:</b></td>
                 <%set equipe=conn.execute("select descricao from equipe where codigo="&exibir("cod_equipe"))%>
              <td class="tb" colspan=5><%=equipe("descricao")%></td>
            </tr>
            <tr>
          ...
          <table width=100% border=0 bgcolor=#E8E8E8 cellspacing="0" cellpadding="0">
            <tr> 
              ...
              <td class="tb"><B>CONTRATOS DO CORRETOR</B></td>
            </tr>
          </table>  
          <table width=100%
            ...
            <tr>
              ...
              <td><a href="Imprimir_contrato.asp?currentPage=<%=currentPage%>&ordem=<%=ordem%>&ordem2=<%=ordem2%>&pesquisa=<%=pesquisa%>&opcao=<%=opcao%>&pagina=Detalhe_corretor&codigo2=<%=codigo%>"><img alt="Imprimir Contratos" src="../img/icone-imprimir.gif" border=0></a></td>
            </tr>
            ...
            <%else
               ...
               do While Not itens.EOF and cont<itens.PageSize
                  ...
                  <tr>
                    ...
                    <td class="tb" align=center bgcolor=<%=cor%>><a href="Detalhe_contrato.asp?codigo=<%=itens("codigo")%>&codigo2=<%=codigo%>&pagina=Detalhe_Corretor"><%=itens("numero")%></a></td>
                    ...
                    <td bgcolor=<%=cor%> align=center>
                      <a href="javascript:confirma('<%=request.servervariables("SCRIPT_NaME")%>?currentPage=<%=currentpage%>&ordem=<%=ordem%>&ordem2=<%=ordem2%>&deletar=<%=itens("codigo")%>&pesquisa=<%=pesquisa%>&opcao=<%=opcao%>&cliente=<%=cliente%>&corretor=<%'=corretor%>')"><img src="../img/excluir_n.gif" alt="Excluir" border=0></a>
                      <a href="Alt_contrato.asp?codigo=<%=itens("codigo")%>&codigo2=<%=codigo%>&pagina=Detalhe_Corretor"><img src="../img/editar.gif" alt="Editar" border=0></a>
                      <%set rescisao=conn.execute("select * from recisao where cod_contrato="&itens("codigo")) %>
                      <%if rescisao.eof then%>
                        <a href="Cad_Rescisao.asp?codigo=<%=itens("codigo")%>&codigo2=<%=codigo%>&pagina=Detalhe_Corretor"><img src="../img/recisao.gif" alt="Rescindir" border=0></a>
                      <%else%>
                        <a href="Alt_Rescisao.asp?codigo=<%=itens("codigo")%>&codigo2=<%=codigo%>&pagina=Detalhe_Corretor"><img src="../img/recisao.gif" alt="Rescindir" border=0></a>
                      <%end if%>
                      ...
                      <%if comissao.eof then%>
                        <a href="Cad_comissao.asp?codigo=<%=itens("codigo")%>&codigo2=<%=codigo%>&pagina=Detalhe_Corretor"><img src="../img/comissao.gif" alt="Comissão" border=0></a>
                      <%else%>
                        <a href="Alt_comissao.asp?codigo=<%=itens("codigo")%>&codigo2=<%=codigo%>&pagina=Detalhe_Corretor"><img src="../img/comissao.gif" alt="Comissão" border=0></a>
                      <%end if%>
                    </td>
                  </tr>
                  <%cont=cont+1%>
