...
itens.PageSize = 3
if ordem="" then
   ordem="bairro.descricao"
end if
if pesquisa<>"" then
  if radio = "1" then 
    sql="and bairro.descricao like '%"&pesquisa&"%' "
  else
    if radio = "2" then
...
itens.Open "select bairro.codigo as codigo , bairro.descricao as bairro , cidade.descricao as cidade , estado.descricao as estado from ((bairro INNER JOIN cidade on bairro.cod_cidade=cidade.codigo)INNER JOIN estado on cidade.cod_estado=estado.codigo) where bairro.status=0 "&sql&" ORDER BY "&ordem&" "&ordem2,conn%>
<html>
<head>
<title></title>
  ...
  <tr>
    <td class="navdroplist">
      <table width=100% align="center" bgcolor="#FFFFFF">
        <tr>
          <td class="navdroplist">
            <a href="main_bairro.asp?contrario=<%=contrario%>&codigo=<%=codigo%>"><img src="../img/logopb.gif" border=0></a>
          </td>
        </tR>
      </table>
      <br>
      <table width=100% align="center" bgcolor="#f7f7f7" border=0>
        <tr>
          <td class="navdroplist" align=center><B>RELATÓRIO BAIRRO(S)</B></td>
        </tR>
      </table>
      <br>
