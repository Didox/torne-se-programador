...
if request("flag") <> 1 then
   set Funcao=conn.execute("select * from Funcao where codigo="&codigo)
   descricao=replace(trim(ucase(Funcao("descricao"))),"'","")
else
   descricao=trim(ucase(request("descricao")))
   if descricao<>"" then
      set verifica_descricao=conn.execute("select * from Funcao where descricao='"&descricao&"' and codigo<>"&codigo)
      if not verifica_descricao.eof then
         msg=msg&" Função já existe<br>"
      end if
   else
      msg=msg&" Função.<br>"
   end if
   if msg="" then
      conn.execute("update Funcao set descricao='"&descricao&"' where codigo="&codigo)%>
      <script>
    ...
    <td>
      ...
      <table border="0" align=left width=380 cellspacing="0" cellpadding="0">
        <tr>
          <td>
            <br>
            <span span class="estilo">&nbsp;&nbsp;<b>Para alterar um Função siga os passos descrito abaixo.<br><br></b>
      ...
      <table border="0" width=376 align=right>
        <tr>
          <td span class="estilo" width=100%>
            <img src="../img/r_14.gif">&nbsp;ALTERAR FUNÇÃO<br><br>
          </td>
