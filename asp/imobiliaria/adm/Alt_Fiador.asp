...
if request("flag") <> 1 then
   set fiador=conn.execute("select * from fiador where codigo="&codigo)
   tipo=fiador("tipo")
   nome=fiador("nome")
   logradouro=fiador("endereco")
   numero=fiador("numero")
   compl=fiador("complemento")
   bairro=fiador("bairro")
   cidade=fiador("cidade")
   estado=fiador("estado")
   cep=fiador("cep")
   ddd_fone=fiador("ddd_fone")
   fone=fiador("fone")
   dddc=fiador("ddd_cel")
   celular=fiador("celular")
   cpf=fiador("cpf")
   banco=fiador("cod_banco")
   agencia=fiador("agencia")
   conta=fiador("conta")
   renda=trim(formatnumber(csng(fiador("renda")),2))
   profissao=fiador("profissao")
   email=fiador("email")
   cartorio=fiador("cartorio")
   renda_extra=trim(formatnumber(csng(fiador("renda_extra")),2))
   data_admissao=data_completa(fiador("data_admissao"))
   empresa=fiador("empresa")
   tipo_imovel=fiador("tipo_imovel")
   if fiador("tipo")=1 then
      identidade=fiador("identidade")
      estado_civil=fiador("estado_civil") 
      escolaridade=fiador("cod_escolaridade")
      sexo=fiador("sexo")
      data_nascimento=data_completa(fiador("data_nascimento"))
      data_emissao=data_completa(fiador("data_emissao"))
      orgao_emissor=fiador("orgao_emissor")
   end if
   ...
   compl=replace(trim(ucase(request("complemento"))),"'","")
   ...
   dddc=replace(trim(ucase(request("ddd_cel"))),"'","")
   ...
   renda=trim(ucase(request("renda")))
   ...
   if cpf<>"" then
      ...
      if tipo=1 then
         if not CalculaCPF(cpf) then
            ...
            set verifica_cpf=conn.execute("select * from fiador where cpf='"&cpf&"' and codigo<>"&codigo)
   ...
   if msg="" then
      ...
      cpf=replace(cpf,"/","")
      if tipo = "1" then
         conn.execute("update fiador set nome='"&nome&"' , tipo="&tipo&" , endereco='"&logradouro&"', numero = '"&numero&"' , complemento = '"&compl&"' , bairro = '"&bairro&"' , cidade = '"&cidade&"' , estado = '"&estado&"' , cep = '"&cep&"' , ddd_fone = "&ddd_fone&" , fone = '"&fone&"' , ddd_cel = '"&dddc&"' , celular = '"&celular&"' , cpf = '"&cpf&"' , identidade = '"&identidade&"' , cod_banco = "&banco&" , agencia = '"&agencia&"' , conta = '"&conta&"', renda = "&replace(replace(csng(renda),".",""),",",".")&" , profissao = '"&profissao&"' , email = '"&email&"' , cod_escolaridade="&escolaridade&" , estado_civil="&estado_civil&", data_nascimento=convert(datetime,'"&data_nascimento&"',103),sexo="&sexo&" , data_emissao=convert(datetime,'"&data_emissao&"',103) , orgao_emissor='"&orgao_emissor&"' , empresa='"&empresa&"',cartorio='"&cartorio&"',renda_extra="&replace(replace(csng(renda_extra),".",""),",",".")&",data_admissao=convert(datetime,'"&data_admissao&"',103), tipo_imovel="&tipo_imovel&" where codigo="&codigo)
      else
         conn.execute("update fiador set nome='"&nome&"' , tipo="&tipo&" , endereco='"&logradouro&"', numero = '"&numero&"' , complemento = '"&compl&"' , bairro = '"&bairro&"' , cidade = '"&cidade&"' , estado = '"&estado&"' , cep = '"&cep&"' , ddd_fone = "&ddd_fone&" , fone = '"&fone&"' , ddd_cel = '"&dddc&"' , celular = '"&celular&"' , cpf = '"&cpf&"' , cod_banco = "&banco&" , agencia = '"&agencia&"' , conta = '"&conta&"', renda = "&replace(replace(csng(renda),".",""),",",".")&" , profissao = '"&profissao&"' , email = '"&email&"' ,  empresa='"&empresa&"',cartorio='"&cartorio&"',renda_extra="&replace(replace(csng(renda_extra),".",""),",",".")&",data_admissao=convert(datetime,'"&data_asmissao&"',103), tipo_imovel="&tipo_imovel&" where codigo="&codigo)
      end if%>
      <script>
    ...
    <td>
      ...
      <table border="0" align=left width=380 cellspacing="0" cellpadding="0">
        <tr>
          <td>
            <br>
            <span span class="estilo">&nbsp;&nbsp;<b>Para alterar os dados fiador siga os passos descrito abaixo.<br><br></b>
      ...
      <table border="0" width=376 align=right>
        <tr>
          <td span class="estilo" width=100%>
            <img src="../img/r_13.gif">&nbsp;ALTERAR DADOS DO FIADOR
          </td>
        ...
        <tr>
         <td>
          <table cellspacing=0 cellpadding=0>
           ...
           <tr>
             <td>
              <input type="text" name="renda_extra" value="<%=renda_extra%>" label="renda extra" style="width:60;height:17;background:#E8E8E8;text-align: RIGHT;" maxlength="18" onkeypress="Numero();" onkeyup="Moeda(this);">&nbsp;
             </td>
             <td>
