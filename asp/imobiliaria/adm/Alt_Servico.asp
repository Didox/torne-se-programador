...
if request("flag") <> 1 then
   set servico=conn.execute("select * from servico where codigo="&codigo)
   descricao=replace(trim(ucase(servico("descricao"))),"'","")
else
   descricao=trim(ucase(request("descricao")))
   if descricao<>"" then
      set verifica_descricao=conn.execute("select * from servico where descricao='"&descricao&"' and codigo<>"&codigo)
      if not verifica_descricao.eof then
         msg=msg&" Serviço já existe<br>"
      end if
   else
      msg=msg&" Serviço.<br>"
   end if
   if msg="" then
      conn.execute("update servico set descricao = '"&descricao&"' where codigo= "&codigo)%>
      <script>
    ...
    <td>
      ...
      <table border="0" align=left width=380 cellspacing="0" cellpadding="0">
        <tr>
          <td>
            <br>
            <span span class="estilo">&nbsp;&nbsp;<b>Para cadastrar um novo serviço siga os passos descrito abaixo.<br><br></b>
      ...
      <table border="0" width=376 align=right>
        <tr>
          <td span class="estilo" width=100%>
            <img src="../img/r_14.gif">&nbsp;CADASTRAR NOVO SERVIÇO<br><br>
          </td>
