...
if request("flag") <> 1 then
   set bairro=conn.execute("select * from bairro where codigo="&codigo)
   descricao=replace(trim(ucase(bairro("descricao"))),"'","")
   cidade   =trim(ucase(bairro("cod_cidade")))
else
   ...
   if descricao<>"" then
      set verifica_descricao=conn.execute("select * from bairro where descricao='"&descricao&"' and codigo<>"&codigo&" and cod_cidade="&cidade)
      if not verifica_descricao.eof then
         msg=msg&" Bairro já existe<br>"
      end if
   else
      msg=msg&" Bairro.<br>"
   end if
   if msg="" then
      conn.execute("update bairro set descricao='"&descricao&"', cod_cidade="&cidade&" where codigo="&codigo)%>
      <script>
      ...
      <table border="0" width=376 align=right>
        <tr>
          <td span class="estilo" width=100%>
            <img src="../img/r_14.gif">&nbsp;ALTERAR BAIRRO<br><br>
          </td>
        ...
        <tr>
          <td>
            <select name="cidade" style="width:140;background=#E8E8E8;">
             <option value = ""></option>
             <%set ccid=conn.execute("select * from cidade where status=0 ORDER BY descricao")%>
             <%do while not ccid.eof %>
              <%if cstr(ccid("codigo"))=cstr(cidade) then%>
                 <option value="<%=ccid("codigo")%>" selected><%=ccid("descricao")%>
              <%else%>
                 <option value="<%=ccid("codigo")%>"><%=ccid("descricao")%>
              <%end if%>
              <%ccid.movenext%>
             <%loop%>
             </select>
