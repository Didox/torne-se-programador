...
<%pagina=request("pagina")
  ...
  set conta=conn.execute("select * from contas where tipo=1 and codigo="&cod_conta)
if request("flag") <> 1 then
  tipo=conta("tipo")
  valor=trim(formatnumber(csng(conta("valor")),2))
  tipo_conta=conta("cod_tipo_conta")
  setor=conta("cod_setor")
  data=data_completa(conta("data"))
  codigo=conta("cod_funcionario")
  ...
  if msg="" then
     conn.execute("update contas set cod_tipo_conta="&tipo_conta&", cod_setor="&setor&", data=convert(datetime,'"&data&"',103), numero='"&numero&"', descricao='"&descricao&"', valor="&replace(replace(csng(valor),".",""),",",".")&" where codigo="&conta("codigo"))%>
       <script>
          alert('Alteração efetuada com sucesso!');
          location.href="<%=pagina%>.asp?codigo=<%=codigo%>&cod_conta=<%=cod_conta%>";
       </script>
  <%end if
end if%>
<html>
...
<form method=post action="<%=request.servervariables("SCRIPT_NAME")%>?flag=1&pagina=<%=pagina%>&codigo=<%=codigo%>&cod_conta=<%=cod_conta%>" name="form1">
      ...
      <table border="0" width=376 align=right>
        ...
        <tr>
          <td>
            <select name="tipo_conta" style="width:140;background=#E8E8E8;">
              <option value = ""></option>
              <%set cbai=conn.execute("select * from tipo_conta where status=0 ORDER BY descricao")%>
       ...
       <tr>
         <td>
           ...
           <tr>
            <td>
             <input type="text" name="valor" value="<%=valor%>" label="valor" style="width:90; height:17;background:#E8E8E8;text-align: RIGHT;" maxlength=18 onkeypress="Numero();" onkeypress="Moeda(this);">
            </td>
         </tr>
         <tr>
           <td>
