...
if request("flag") = 1 then
  ...
  if msg="" then
    conn.execute("insert into recisao (cod_contrato , cod_motivo, descricao) values ("&contrato("numero")&", "&tipo_motivo&", '"&descricao&"')")%>
     <script>
      ...
      <table border="0" width=376 align=right>
       <tr>
          <td span class="estilo" width=100%>
            <img src="../img/r_14.gif">&nbsp;CADASTRAR RESCISÃO
          </td>
       ...
       <tr>
         <td>
           <table cellspacing=0 cellpadding=0>
            <tr>
             <td>
               <img src="../img/obrigacao.gif">&nbsp;<span class="estilo">Tipo da Rescisão:
             </td>
           </tr>    
           <tr>
            <td>
              <select name="tipo_motivo" style="width:200;background=#E8E8E8;">
                <option value = ""></option>
                 <%set cbai=conn.execute("select * from tipo_motivo ORDER BY descricao")%>
                 <%do while not cbai.eof %>
                    <%if cstr(cbai("codigo"))= tipo_motivo then%>
