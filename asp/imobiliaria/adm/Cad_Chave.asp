...
if request("flag") = 1 then
   ...
   if msg="" and reload="" then
      conn.execute("insert into registro_chave (cod_imovel,cod_quadro,cod_localizacao,descricao,data_inclusao) values ("&imovel&","&quadro&","&localizacao&",'"&descricao&"',convert(datetime,'"&date()&"',103))")%>
      <script>
        ...
        <tr>
           <td colspan=2>
             <select name="quadro" style="background=#E8E8E8;" <%if local<>"1" then%>disabled<%end if%>> 
               <option value = ""></option>
                 <%set quadros  =conn.execute("SELECT *  FROM Quadro  WHERE codigo NOT IN  (SELECT cod_quadro FROM registro_chave WHERE status = 0)")%>
