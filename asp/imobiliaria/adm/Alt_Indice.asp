...
if request("flag") <> 1 then
   set indice = conn.execute("select * from indice where codigo = "&codigo)
   descricao=request("descricao")
   valor=trim(formatnumber(csng(request("valor")),2))
else
   descricao=replace(trim(ucase(request("descricao"))),"'","")
   valor=trim(ucase(request("valor")))
   if descricao<>"" then
      set verifica_descricao=conn.execute("select * from indice where descricao='"&descricao&"' and codigo<>"&codigo)
      if not verifica_descricao.eof then
         msg=msg&" Índice já existe<br>"
      end if
   else
      msg=msg&" Índice.<br>"
   end if
   ...
   set valida_indice = conn.execute("select * from indice where descricao = '"&descricao&"' and data_atualizada = '"&data_atualizada&"'")
   if valida_indice.eof then
      if msg="" then
        conn.execute("update indice set descricao = '"&descricao&"', valor = "&replace(replace(csng(valor),".",""),",",".")&", data_atualizada = convert(datetime,'"&date()&"',103) where codigo="&codigo)%>
        <script>
    ...
    <td>
      ...
      <table border="0" align=left width=380 cellspacing="0" cellpadding="0">
        <tr>
          <td>
            <br>
            <span span class="estilo">&nbsp;&nbsp;<b>Para alterar um índice siga os passos descrito abaixo.<br><br></b>
      ...
      <table border="0" width=376 align=right>
        <tr>
          <td span class="estilo" width=100%>
            <img src="../img/r_14.gif">&nbsp;ALTERAR ÍNDICE
          </td>
        ...
        <tr>
          <td>
            <input type="text" name="descricao" value="<%=indice("descricao")%>" maxlength=50 label="Descrição" style="width:290; height:17; background=#E8E8E8;">
          </td>
        </tr>
        <tr>
          <td>
        ...
        <tr>
          <td>
            <input type="text" name="valor" value="<%=indice("valor")%>" label="valor" style="width:60; height:17;background:#E8E8E8;text-align: RIGHT;" maxlength=18 onkeypress="Numero();" onkeyUp="Moeda(this);">
          </td>
        </tr>
        <tr>
          <td>
