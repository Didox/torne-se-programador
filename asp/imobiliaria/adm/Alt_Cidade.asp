...
if request("flag") <> 1 then
   set cidade=conn.execute("select * from cidade where codigo="&codigo)
   descricao=replace(trim(ucase(cidade("descricao"))),"'","")
   estado   =trim(ucase(cidade("cod_estado")))
else
   descricao=replace(trim(ucase(request("descricao"))),"'","")
   estado   =trim(ucase(ucase(request("estado"))))
   if estado="" then
     msg=msg&" Estado.<br>"
   end if
   if descricao<>"" then
      set verifica_descricao=conn.execute("select * from cidade where descricao='"&descricao&"' and codigo<>"&codigo&" and cod_estado="&estado)
      if not verifica_descricao.eof then
         msg=msg&" Cidade já existe<br>"
      end if
   else
      msg=msg&" Cidade.<br>"
   end if
   if msg="" then
      conn.execute("update cidade set descricao = '"&descricao&"' , cod_estado="&estado&" where codigo="&codigo)%>
      <script>
    ...
    <td>
      ...
      <table border="0" align=left width=380 cellspacing="0" cellpadding="0">
        <tr>
          <td>
            <br>
            <span span class="estilo">&nbsp;&nbsp;<b>Para alterar a cidade siga os passos descrito abaixo.<br><br></b>
      ...
      <table border="0" width=376 align=right>
        <tr>
          <td span class="estilo" width=100%>
            <img src="../img/r_14.gif">&nbsp;ALTERAR CIDADE<br><br>
          </td>
