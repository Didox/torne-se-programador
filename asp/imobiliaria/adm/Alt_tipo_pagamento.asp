...
if request("flag") <> 1 then
   set tipo_pagamento=conn.execute("select * from tipo_pagamento where codigo="&codigo)
   descricao=replace(trim(ucase(tipo_pagamento("descricao"))),"'","")
else
   descricao=trim(ucase(request("descricao")))
   if descricao<>"" then
      set verifica_descricao=conn.execute("select * from tipo_pagamento where descricao='"&descricao&"' and codigo<>"&codigo)
      if not verifica_descricao.eof then
         msg=msg&" Tipo de Pagamento já existe<br>"
      end if
   else
      msg=msg&" Tipo de Pagamento.<br>"
   end if
   if msg="" then
      conn.execute("update tipo_pagamento set descricao='"&descricao&"' where codigo="&codigo)%>
      <script>
    ...
    <td>
      ...
      <table border="0" align=left width=380 cellspacing="0" cellpadding="0">
        <tr>
          <td>
            <br>
            <span span class="estilo">&nbsp;&nbsp;<b>Para alterar um tipo de pagamento siga os passos descrito abaixo.<br><br></b>
