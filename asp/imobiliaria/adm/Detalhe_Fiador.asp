...
itens.Open "SELECT contrato.*, contrato.codigo AS codigo, contrato.data_vencimento as vencimento, contrato.numero AS numero, imoveis.descricao AS imovel, clientes.nome AS nome, servico.descricao AS servico, contrato.status AS status FROM (((contrato INNER JOIN clientes ON contrato.cod_cliente = clientes.codigo) INNER JOIN imoveis ON contrato.cod_imovel = imoveis.codigo) INNER JOIN servico ON contrato.tipo_servico = servico.codigo) where cod_fiador1="&codigo&" or cod_fiador2="&codigo&" and contrato.status=0 "&sql&" ORDER BY "&ordem&" "&ordem2,conn%>
<html>
<head>
<title>:::::Imobi....:::</title>
      ...
      <!--#INCLUDE FILE ="Main_Topo.asp"-->
          ...
          <table width=100% border=0 bgcolor=#E8E8E8 cellspacing="0" cellpadding="0">
            <tr> 
              ...
              <td class="tb" align=right><a href="Imprimir_detalhe_fiador.asp?codigo=<%=exibir("codigo")%>&pagina=Detalhe_fiador"><img src="../img/icone-imprimir.gif" border=0></a></td>
            </tr>
          </table>
          ...
          <table width=100% border=0 cellspacing="0" cellpadding="0">
            ...
            <tr>
              ...
              <td class="tb">
                 <%set bancos=conn.execute("select * from bancos where codigo="&exibir("cod_banco"))%>
                 <%=bancos("descricao")%>
              </td>
          ...
          <table width=100%
            ...
            <tr>
              ...
              <td><a href="Imprimir_contrato.asp?currentPage=<%=currentPage%>&ordem=<%=ordem%>&ordem2=<%=ordem2%>&pesquisa=<%=pesquisa%>&opcao=<%=opcao%>&pagina=Detalhe_fiador&codigo2=<%=codigo%>"><img alt="Imprimir Contratos" src="../img/icone-imprimir.gif" border=0></a></td>
            </tr>
               ...
               do While Not itens.EOF and cont<itens.PageSize
                  ...
                  <tr>
                    ...
                    <td bgcolor=<%=cor%> align=center>
                      ...
                      <a href="Alt_contrato.asp?codigo=<%=itens("codigo")%>&codigo2=<%=codigo%>&pagina=Detalhe_Fiador"><img src="../img/editar.gif" alt="Editar" border=0></a>
                      <%set rescisao=conn.execute("select * from recisao where cod_contrato="&itens("codigo")) %>
                      <%if rescisao.eof then%>
                        <a href="Cad_Rescisao.asp?codigo=<%=itens("codigo")%>&codigo2=<%=codigo%>&pagina=Detalhe_Fiador"><img src="../img/recisao.gif" alt="Rescindir" border=0></a>
                      <%else%>
                        <a href="Alt_Rescisao.asp?codigo=<%=itens("codigo")%>&codigo2=<%=codigo%>&pagina=Detalhe_Fiador"><img src="../img/recisao.gif" alt="Rescindir" border=0></a>
                      <%end if%>
                      ...
                      <%if comissao.eof then%>
                        <a href="Cad_comissao.asp?codigo=<%=itens("codigo")%>&codigo2=<%=codigo%>&pagina=Detalhe_Fiador"><img src="../img/comissao.gif" alt="Comissão" border=0></a>
                      <%else%>
