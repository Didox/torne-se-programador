...
if request("flag") <> 1 then
   set equipe=conn.execute("select * from equipe where codigo="&codigo)
   descricao=replace(trim(ucase(equipe("descricao"))),"'","")
   supervisor=trim(ucase(equipe("cod_supervisor")))
else
   descricao=trim(ucase(request("descricao")))
   supervisor=trim(ucase(request("supervisor")))
   if descricao<>"" then
      set verifica_descricao=conn.execute("select * from equipe where descricao='"&descricao&"' and codigo<>"&codigo)
      if not verifica_descricao.eof then
         msg=msg&" Equipe já existe<br>"
      end if
   else
      msg=msg&" Equipe.<br>"
   end if
   if msg="" then
      conn.execute("update equipe set descricao = '"&descricao&"' , cod_supervisor="&supervisor&" where codigo="&codigo)%>
      <script>
    ...
    <td>
      ...
      <table border="0" align=left width=380 cellspacing="0" cellpadding="0">
        <tr>
          <td>
            <br>
            <span span class="estilo">&nbsp;&nbsp;<b>Para alterar uma equipe siga os passos descrito abaixo.<br><br></b>
      ...
      <table border="0" width=376 align=right>
        <tr>
          <td span class="estilo" width=100%>
            <img src="../img/r_14.gif">&nbsp;ALTERAR EQUIPE<br><br>
          </td>
        ...
        <tr>
          <td>
            <img src="../img/obrigacao.gif">&nbsp;<span class="estilo">Supervisor:
          </td>
        </tr>
        <tr>
          <td>
       <select name="supervisor" style="width:290;background=#E8E8E8;">
             <option value = ""></option>
             <%set cbai=conn.execute("select * from corretor where status=0 ORDER BY nome")%>
             <%do while not cbai.eof %>
              <%if cstr(cbai("codigo"))=cstr(supervisor) then%>
