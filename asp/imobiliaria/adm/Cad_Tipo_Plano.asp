...
if request("flag") = 1 then
   ...
   if descricao<>"" then
      set verifica_descricao=conn.execute("select * from tipo_conta where descricao='"&descricao&"'")
      if not verifica_descricao.eof then
         msg=msg&" plano já existe<br>"
      end if
   else
      msg=msg&" plano.<br>"
   end if
   ...
   if msg="" then
      conn.execute("insert into tipo_conta (descricao,meses,tipo) values ('"&descricao&"',"&meses&",0)")%>
      <script>
    ...
    <td>
      ...
      <table border="0" align=left width=380 cellspacing="0" cellpadding="0">
        <tr>
          <td>
            <br>
            <span span class="estilo">&nbsp;&nbsp;<b>Para cadastrar um novo tipo de plano siga os passos descrito abaixo.<br><br></b>
      ...
      <table border="0" width=376 align=right>
        <tr>
          <td span class="estilo" width=100%>
            <img src="../img/r_14.gif">&nbsp;CADASTRAR NOVO TIPO DE PLANO<br><br>
          </td>
        ...
        <tr>
          <td>
            <input type="text" name="descricao" value="<%=descricao%>" maxlength=50 label="plano" style="width:290; height:17; background=#E8E8E8;">
          </td>
        </tr>
        <tr>
          <td>
