    ...
    public class Diretorios
    {
        ...
        public bool ExcluirArquivo()
        {
            try
            {
				...
				string path = HttpUtility.UrlDecode(AppDomain.CurrentDomain.BaseDirectory + ConfigurationSettings.AppSettings["dirMain"]);
                ...
                if (Path.GetExtension(voltaDir) != "")
                {
					...
					{
						this._ret[0] = "0";
						this._ret[1] = "Erro ao excluir arquivo, não localizado";
						return false;
					}
                }
                else
            ...
            try
            {
                ...
                for (int i = 0; i < arq.Length; i++)
                {
                    if (arq[i].Name != "Thumbs.db")
                    {
                        ...
                        dr["nome"] = "<a href=\"javascript:void(0)\" onclick=\"VerImagem('" + voltaDir + "//" + arq[i].Name+ "'); \" ><img src=imagens/ftp/" + strIcone + " alt=\"Clique para baixar\" border=0 >&nbsp;&nbsp;" + arq[i].Name + "</a>";
