...
begin
  ...
  begin
     MessageDlg('Digite o Endereço',mtinformation, [mbOK], 0);
     valido := false;
  ...
  begin
     MessageDlg('Digite o Número',mtinformation, [mbOK], 0);
     valido := false;
     maskNumero.SetFocus;
  end;
...
begin
  ...
  try
    ...
    begin
       MessageDlg('CPF de cliente já cadastrado',mtError, [mbOK], 0);
    end
    else
    begin
        if trim( edtNome.Text ) = '' then
